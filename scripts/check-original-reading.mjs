...
assert.equal(catalog.articles.length, 41)
const ids = new Set()
for (const article of catalog.articles) {
  ...
  ids.add(article.id)
  assert.equal(article.contentVersion, '3.0.0')
...
assert.ok(!reader.includes('sourceTitles') && !reader.includes('allowSourceLinks'), 'Article navigation and body do not display source prompts')
