...
check('resource selection is URL-controlled and invalid IDs recover', () => {
  for (const fragment of ["route.resourceEntry && !['learn', 'analyze', 'problems', 'all', 'read', 'cases', 'library'].includes(route.resourceEntry)", '<ResourceTopicRoute', 'requestedEntryId={route.resourceEntry!}', "onReplaceInvalidEntry={() => navigateResourceEntry('problems', true)}", 'requestedEntryId={route.resourceEntry}']) assert.ok(appSource.includes(fragment), `missing ${fragment}`)
})
