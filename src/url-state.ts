...
export function parseRouteHash(hash: string): AppRoute {
  ...
  if (view === 'course') {
    ...
    return routeWithContext({ view: 'course', tool: DEFAULT_ROUTE.tool }, readContext(search, pathContext))
  }
  if (view === 'workbench') {
  ...
  if (view === 'resources') return routeWithContext({ view: 'resources', tool: DEFAULT_ROUTE.tool, ...(first ? { resourceEntry: first } : {}), ...(second ? { resourceId: second } : {}) }, context)
  if (view === 'method') {
...
export function serializeRoute(route: AppRoute): string {
  if (route.view === 'course') {
    ...
    const path = context?.unitId ? `#course/units/${encodeURIComponent(context.unitId)}${context.activityId ? `/activities/${encodeURIComponent(context.activityId)}` : ''}` : '#course'
...
export function routeTitle(route: AppRoute): string {
  if (route.view === 'course') return withBrand(route.workContext?.unitId ? `课程单元 ${route.workContext.unitId}` : '系统课程')
