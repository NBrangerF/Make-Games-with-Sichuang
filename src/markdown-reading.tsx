...
function renderInline(source: string, keyPrefix: string): ReactNode[] {
  ...
  while ((match = token.exec(source))) {
    ...
    if (match[2] !== undefined && match[3]) {
      ...
      nodes.push(<span className="reading-source-reference" title={`原链接：${match[5]}`} key={key}>{match[4]}</span>)
    } else if (match[6]) {
...
export function MarkdownReading({ source }: MarkdownReadingProps) {
    ...
    if (line.trim().startsWith('|') && index + 1 < lines.length && isTableDivider(lines[index + 1])) {
      ...
      blocks.push(
        <div className="reading-table-wrap" key={`table-${index}`}>
          <table>
            <thead><tr>{header.map((cell, cellIndex) => <th key={`head-${cellIndex}`}>{renderInline(cell, `head-${cellIndex}`)}</th>)}</tr></thead>
            <tbody>{rows.map((row, rowIndex) => <tr key={`row-${rowIndex}`}>{row.map((cell, cellIndex) => <td key={`cell-${rowIndex}-${cellIndex}`}>{renderInline(cell, `cell-${rowIndex}-${cellIndex}`)}</td>)}</tr>)}</tbody>
          </table>
        </div>,
      )
      continue
