...
export function TextLearning({ chapterId, language = 'zh-CN', returnTo, onOpenTool }: { chapterId?: string; language?: Language; returnTo?: string; onOpenTool: (id: GuideToolId) => void }) {
  ...
  const contextualLibrary = libraryEntries.filter(item => item.chapterIds.includes(chapterId || ''))
  ...
  return <main id="main-content" tabIndex={-1} lang={language} className={`text-learning${chapterId ? ' text-learning--chapter' : ''}`}>
    <nav className="original-reading__bar" aria-label={c.label}>
      <div className="original-reading__backlinks">{chapterId && returnTo && <a href={returnTo}>{fromLibrary ? (language === 'en' ? '← Back to the library entry' : '← 返回刚才的条目') : fromCases ? (language === 'en' ? '← Back to the case' : '← 返回刚才的案例') : (language === 'en' ? '← Back to articles' : '← 返回文章目录')}</a>}<a href={chapterId ? readingHref('course', undefined, language) : `#resources/read/all/${language}`}>{chapterId ? c.toc : c.knowledge}</a></div>
