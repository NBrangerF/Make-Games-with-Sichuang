...
export function DesignLibrary({ entryId, language = 'zh-CN', query = '', kind, question, returnTo, onFilterChange }: { entryId?: string; language?: ReadingLanguage; query?: string; kind?: LibraryKind; question?: LibraryQuestion; returnTo?: string; onFilterChange: (query: string, kind?: LibraryKind, question?: LibraryQuestion) => void }) {
  ...
  const companion = entry && <div className="library-connections">
    ...
    {entry.chapterIds.map(originalId => { const id = chapterForTrack(originalId, trackFromReturn(returnTo)); const article = articles.articles.find(item => item.id === id); return article && <p key={id}><a href={readingHref('course', id, language, { readingReturnTo: ownHref })}>{article.title[language]}</a></p> })}
    {entry.caseIds.length > 0 && <h3>{t('真实游戏中的做法', 'In real games')}</h3>}
    {entry.caseIds.map(id => { const game = designCases.find(item => item.id === id); return game && <p key={id}><a href={readingHref('cases', id, language, { readingReturnTo: ownHref })}>{caseGameName(game, language)} · {game.title[language]}</a></p> })}
  ...
  return <main id="main-content" tabIndex={-1} lang={language} className={`original-reading design-library${collection ? " design-library--collection" : ""}`}>
    ...
    {collection ? <>
      <header className="original-reading__intro library-intro"><p>{t('看懂规则怎样工作', 'See how the rules work')}</p><h1>{t('机制与主题', 'Mechanisms & themes')}</h1><p>{t('查一个机制，走一段例子，再看看怎样用到自己的草稿里。', 'Look up a mechanism, work through an example, and consider how it might fit your draft.')}</p></header>
      <div className="library-discovery-layout"><div className="library-discovery-main">
