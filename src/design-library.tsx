...
export function DesignLibrary({ entryId, language = 'zh-CN', query = '', kind, question, returnTo, onFilterChange }: { entryId?: string; language?: ReadingLanguage; query?: string; kind?: LibraryKind; question?: LibraryQuestion; returnTo?: string; onFilterChange: (query: string, kind?: LibraryKind, question?: LibraryQuestion) => void }) {
  ...
  const filtered = searchLibrary(libraryEntries, query, kind, question)
  ...
  return <main id="main-content" tabIndex={-1} lang={language} className="original-reading design-library">
    ...
    </> : entry ? <><p className="library-entry-kind">{kindLabels[libraryGroup(entry.kind)][language]} · {questionLabels[entry.question][language]}</p><ArticleBody key={`${entry.id}/${language}`} article={entry} language={language} bodyLoader={bodies[`../content/design-library/${entry.id}/${language}.md`]} allowSourceLinks withOutline companion={companion}/><nav className="original-reading__end" aria-label={t('继续阅读', 'Continue reading')}><a href={backHref}>← {backLabel}</a><a href={listHref}>{t('全部条目', 'All entries')} →</a></nav></> : <section><h1>{t('没有找到这个条目', 'Entry not found')}</h1><p>{t('链接可能已经改变。可以从目录继续查找。', 'The link may have changed. Continue from the library.')}</p><a href={listHref}>{t('返回资料库', 'Back to the library')}</a></section>}
  </main>
}
