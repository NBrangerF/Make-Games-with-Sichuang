...
适用于 `49e9317` 之后的阅读实现。2026-09-11：96 篇库文章与 18 篇案例完成，见[最终验收](2026-09-11-library-acceptance.md)。正文审核记录保存在仓库之外，维护者需要同时取得内部研究交接包。只有 Git 仓库时可以运行、检查和改进页面，不能据此声称已读原书或完成新归因的回源核查。
...
`aliases` 保留条目的替代名称；例子、子型、功能与宽泛问题放入 `searchTerms`。二者都参与搜索，但都不自动转换或合并稳定 ID。改变字段归属时检查旧查询仍可命中，并运行 `pnpm copy:audit` 更新生成的内容扫描指纹。
...
内部交接包中的当前入口为 `执行清单.json`、`核查/C09-final/最终验收总表.md`、`十九批内容验收.json`、`设计师来源验收.json`、`关系语义复核.md`、`交互示例验收.json`、`当前正文与证据核对.json`、`历史审核标记解释.json`、`索引/BuildingBlocks-203-使用与覆盖.json` 和每批 `发布结果.md`。除明确带目录的文件外，验收文件均在 `核查/C09-final/`。文件名用于内部交接定位；不要将这些文件复制到仓库。
